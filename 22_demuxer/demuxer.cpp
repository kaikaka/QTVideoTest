...
//初始化解码器
int Demuxer::initDecode(AVCodecContext **decodeCtx, int *streamIdx, AVMediaType type) {
    //根据type寻找最合适的流信息
    //返回值是流索引
...
int Demuxer::initAudioInfo() {
    //初始化解码器
    int ret = initDecode(&_aDecodeCtx,&_aStreamIdx,AVMEDIA_TYPE_AUDIO);
    RET(initDecode);
    //打开文件
...
int Demuxer::initVideoInfo() {
    //初始化解码器
    int ret = initDecode(&_vDecodeCtx,&_vStreamIdx,AVMEDIA_TYPE_VIDEO);
    RET(initDecode);
    //打开文件
...
void Demuxer::writeAudioFrame() {
    //libfdk_aac 解码器，解码出来的pcm格式：s16
    //aac 解码器，解码出来的pcm格式：ftlp
    if (av_sample_fmt_is_planar(_aOut->sampleFmt)) {
        //planar
        //外层循环：每一个声道的样本数 si = sample index
        for (int si = 0;si < _frame->nb_samples;si++) {
            //内层循环：有多少个声道 ci = channl index
